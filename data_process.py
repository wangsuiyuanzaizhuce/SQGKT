...
if __name__ == "__main__":
    processed_path = "data/data_processed.csv"
    if os.path.exists(processed_path):
        data = pd.read_csv(processed_path, encoding="ISO-8859-1")
    else:
        data = pd.read_csv(
            filepath_or_buffer="data/skill_builder_data_corrected_collapsed.csv", encoding="ISO-8859-1"
        )
        ...
        question_attempt_stats = (
            data.groupby("problem_id")["attempt_count"].mean().reset_index()
        )
        question_attempt_stats.rename(
            columns={"attempt_count": "mean_attempt"}, inplace=True
        )
        data = pd.merge(
            data, question_attempt_stats, on="problem_id", suffixes=("", "_attempt")
        )
        data["attempt_factor"] = 1 - poisson(data["mean_attempt"]).cdf(
            data["attempt_count"] - 1
        )
        data["attempt_factor_g"] = k + (1 - k) / (
                1 + np.exp(-d * (data["attempt_factor"] - b))
        )

        question_hint_stats = (
            data.groupby("problem_id")["hint_count"].agg("mean").reset_index()
        )
        ...
        data["hint_factor_g"] = k + (1 - k) / (
                1 + np.exp(-d * (data["hint_factor"] - b))
        )
        data["ability_factor"] = data.groupby("user_id")["correct"].transform("mean")
        data.to_csv(processed_path, sep=",", index=False)
